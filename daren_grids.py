...
SAVE_GRIDS_DIR = "static/photo/grids"
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for item_dict in data_list:
            for filename, url in item_dict.items():
                tasks.append(asyncio.create_task(download_one(session, filename, url, sem)))
...
def create_9_grid_with_filler(image_paths, filename="final_grid.jpg"):
    if not os.path.exists(SAVE_GRIDS_DIR):
        os.makedirs(SAVE_GRIDS_DIR)
    save_path = os.path.join(SAVE_GRIDS_DIR, filename)
